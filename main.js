...
canvas.addEventListener('mouseup', (e) => {
    ...
    if (type === 'satellite') {
        ...
        if (type === 'star') radius = starmass/1000;
        if (type === 'planet') radius = planetmass/30;
        if (type === 'blackhole') radius = 7;
    }

    const newBody = new CelestialBody(name, type, mouse.startX, mouse.startY, finalVx, finalVy, mass, color, radius);
...
function updateUIObjectsList() {
    ...
    if (universe.length === 0) {
        listContainer.innerHTML = '<p class="hint" style="padding:5px;">No objects in orbit.</p>';
        return;
    }
